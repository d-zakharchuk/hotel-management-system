    ...
    public partial class SuperUser : Form
    {
       ...
       Room ROOMS = new Room();
        ...
        private void ShowServicesTable()
        {
            ctx = new HotelDataEF.HotelManagementDatabaseEntities();
            ctx.Services.Load();
            this.servicesBindingSource.DataSource = ctx.Services.Local.ToBindingList();
        }
        ...
        private void ShowRoomTable()
        {
            ctx = new HotelDataEF.HotelManagementDatabaseEntities();
            ctx.Room.Load();
            this.roomBindingSource.DataSource = ctx.Room.Local.ToBindingList();
        ...
        private void Delete_Service()
        {
            ...
            int AllReservationsCount = ctx.Reservation.Local.Count;
        ...
        private void BeginServiceUpdate()
        {
            ...
            Services SERVICES = new Services(new List<string>(), false);
            List<HotelDataEF.Services> dt = (List<HotelDataEF.Services>)(SERVICES.SelectService(Convert.ToInt16(UpdateServiceIDUpDown.Value)));
            if (dt.Count > 0)
            {
                UpdateServiceNameTextBox.Text = dt[0].Service.ToString();
                UpdateServicePriceTextBox.Text = dt[0].Price.ToString();
            }
        }
        ...
        private void Update_Service()
        {
            if (UpdateServiceNameTextBox.Text.Trim().Length == 0 || UpdateServicePriceTextBox.Text.Trim().Length == 0)
                ...
                try
                {
                    ...
                    Services SERVICES = new Services(new List<string>(), false);
                    SERVICES.UpdateService(id, id, UpdateServiceNameTextBox.Text, Price);
        ...
        private void BeginRoomUpdate()
        {
            ...
            HotelDataEF.Room DesiredRoom = (HotelDataEF.Room)(ROOMS.SelectRoomByNum(Convert.ToInt16(RoomNumTextBox.Text)));
                RoomTypeComboBox.Text = DesiredRoom.type.ToString();
                RoomFloorUpDown.Value = Convert.ToDecimal(DesiredRoom.floor.ToString());
        }
        ...
        private void BeginTypeUpdate()
        {
            ...
            List<HotelDataEF.Type> TypesList = (List<HotelDataEF.Type>)(ROOMS.SelectType(DeleteTypeTextBox.Text));
            if (TypesList.Count > 0)
            {
                UpdateTypeTextBox.Text = TypesList[0].type1.ToString();
                UpdatePeopleUpDown.Value = Convert.ToDecimal(TypesList[0].C_people.ToString());
                UpdatePrice1TextBox.Text = TypesList[0].Price_1.ToString();
                UpdatePriceTextBox.Text = TypesList[0].Price.ToString();
                UpdateAreaTextBox.Text = TypesList[0].Area.ToString();
            }
        }
        ...
        private void Update_Type()
        {
            try
            {
                ...
                {
                    List<HotelDataEF.Type> TypesList = (List<HotelDataEF.Type>)(ROOMS.SelectType(new_type));
                    if (old_type != new_type && TypesList.Count > 0)
        ...
        private void Add_Type()
        {
            try
            {
                ...
                {
                    List<HotelDataEF.Type> TypesList = (List<HotelDataEF.Type>)(ROOMS.SelectType(type));
                    if (TypesList.Count > 0)
                        MessageBox.Show("Type with this name already exists.", "Error", MessageBoxButtons.OK,
                            MessageBoxIcon.Error);
                    else
